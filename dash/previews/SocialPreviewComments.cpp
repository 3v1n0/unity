...
void SocialPreviewComments::SetupViews()
{
  ...
  cr_bubble_.reset(new nux::CairoWrapper(GetGeometry(), sigc::bind(sigc::mem_fun(this, &SocialPreviewComments::RedrawBubble), nux::ButtonVisualState::VISUAL_STATE_PRELIGHT)));
}

void SocialPreviewComments::UpdateBaloonTexture()
{
  nux::Geometry const& geo = GetGeometry();
...
void SocialPreviewComments::RedrawBubble(nux::Geometry const& geom, cairo_t* cr, nux::ButtonVisualState faked_state)
{
  double blur = 4.0;
...
void SocialPreviewComments::DrawBubble(cairo_t* cr,
                   double line_width,
                   double   radius,
                   double   x,
                   double   y,
