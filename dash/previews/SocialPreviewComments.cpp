...
void SocialPreviewComments::SetupViews()
{
  ...
  for (dash::SocialPreview::CommentPtr comment : social_preview_model->GetComments())
  {
    ...
    if (!comment->display_name.empty())
    {
      ...
      comment_name->mouse_click.connect(sigc::mem_fun(this->preview_container_, &PreviewContainer::OnMouseDown));
    ...
    if (!comment->time.empty())
    {
      ...
      comment_time->mouse_click.connect(sigc::mem_fun(this->preview_container_, &PreviewContainer::OnMouseDown));
    ...
    comment_value->mouse_click.connect(sigc::mem_fun(this->preview_container_, &PreviewContainer::OnMouseDown));
