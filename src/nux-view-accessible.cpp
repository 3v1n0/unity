...
/* public */
/*
 * Right now there are two kind of "focused" states:
 *
 * * nux::Focusable class: it is in high-medium abstract level,
 *       "toolkit" if you want it
 *
 * * nux::InputArea: HasKeyboardFocus and others. This is the low
 *     level focus information
 *
 * Focusable in the end requires the main Window having the los level
 * focus information.
 *
 * This focused is the low levek "key input focus"
 *
 */
gboolean
nux_view_accessible_get_key_focused (NuxViewAccessible *self)
{
  g_return_val_if_fail (NUX_IS_VIEW_ACCESSIBLE (self), FALSE);

  return self->priv->focused;
}
