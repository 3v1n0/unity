...
static void unity_panel_home_button_real_allocate (ClutterActor* base, const ClutterActorBox* box, ClutterAllocationFlags flags) {
	...
	lwidth = ((float) unity_shell_get_launcher_width_foobar (self->priv->_shell)) - 1.0f;
...
static void unity_panel_home_button_real_get_preferred_width (ClutterActor* base, float for_height, float* min_width, float* nat_width) {
	UnityPanelHomeButton * self;
	self = (UnityPanelHomeButton*) base;
	*min_width = unity_shell_get_launcher_width_foobar (self->priv->_shell) - 1.0f;
	*nat_width = unity_shell_get_launcher_width_foobar (self->priv->_shell) - 1.0f;
}
...
static GObject * unity_panel_home_button_constructor (GType type, guint n_construct_properties, GObjectConstructParam * construct_properties) {
	...
	_inner_error_ = NULL;
	{
		...
		__catch7_g_error:
		{
			...
			_inner_error_ = NULL;
			{
				g_warning ("panel-home-button.vala:74: Could not load normal-state bg-texture: %s", e->message);
				_g_error_free0 (e);
			}
		}
		__finally7:
		...
		__catch8_g_error:
		{
			...
			_inner_error_ = NULL;
			{
				g_warning ("panel-home-button.vala:84: Could not load prelight-state bg-texture: %" \
"s", e->message);
				_g_error_free0 (e);
			}
		}
		...
		__catch9_g_error:
		{
			...
			_inner_error_ = NULL;
			{
				g_warning ("panel-home-button.vala:94: Could not load active-state bg-texture: %s", e->message);
				_g_error_free0 (e);
			}
		}
		__finally9:
