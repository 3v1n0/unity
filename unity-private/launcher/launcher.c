...
static void _lambda84_ (UnityLauncherLauncher* self);
static void __lambda84__clutter_actor_queue_redraw (ClutterActor* _sender, ClutterActor* origin, gpointer self);
...
static GObject * unity_launcher_launcher_constructor (GType type, guint n_construct_properties, GObjectConstructParam * construct_properties) {
	...
	self = UNITY_LAUNCHER_LAUNCHER (obj);
	{
		...
		g_signal_connect_object ((ClutterActor*) self->priv->view, "queue-redraw", (GCallback) __lambda84__clutter_actor_queue_redraw, self, 0);
	}
	return obj;
}
