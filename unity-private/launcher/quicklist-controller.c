...
static void _lambda23_ (DbusmenuMenuitem* top_menu, UnityLauncherApplicationQuicklistController* self) {
	if (DBUSMENU_IS_MENUITEM (top_menu)) {
		if (dbusmenu_menuitem_get_root (top_menu)) {
			...
			g_warning ("quicklist-controller.vala:221: menu given not a root item");
		}
	}
}
...
static void _lambda30_ (DbusmenuMenuitem* bottom_menu, UnityLauncherApplicationQuicklistController* self) {
	...
	if (DBUSMENU_IS_MENUITEM (bottom_menu)) {
		if (dbusmenu_menuitem_get_root (bottom_menu)) {
			...
			g_warning ("quicklist-controller.vala:246: menu given not a root item");
		}
	}
...
static void unity_launcher_application_quicklist_controller_on_state_change (UnityLauncherApplicationQuicklistController* self) {
	...
	if (((UnityLauncherQuicklistController*) self)->menu == NULL) {
		...
		g_warning ("quicklist-controller.vala:165: state change called on menu when menu d" \
"oes not exist");
	}
...
static void __lambda25__dbusmenu_menuitem_property_changed (DbusmenuMenuitem* _sender, const char* property_name, void* value, gpointer self) {
	_lambda25_ (property_name, value, self);
}
