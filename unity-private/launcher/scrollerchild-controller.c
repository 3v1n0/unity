/* scrollerchild-controller.c generated by valac 0.9.5, the Vala compiler
...
static gboolean _lambda16_ (UnityLauncherScrollerChildController* self);
...
static void _lambda17_ (UnityThemeFilePath* theme, const char* filepath, UnityLauncherScrollerChildController* self);
static void __lambda17__unity_theme_file_path_found_icon_path (UnityThemeFilePath* _sender, const char* filepath, gpointer self);
static void _lambda18_ (Block10Data* _data10_);
static void __lambda18__unity_theme_file_path_failed (UnityThemeFilePath* _sender, gpointer self);
...
static gboolean _lambda16_ (UnityLauncherScrollerChildController* self) {
	gboolean result = FALSE;
	g_debug ("scrollerchild-controller.vala:203: setting menu to menu");
...
static void _lambda17_ (UnityThemeFilePath* theme, const char* filepath, UnityLauncherScrollerChildController* self) {
	...
	__catch45_g_error:
	{
		...
		{
			char* _tmp2_;
			g_warning ("scrollerchild-controller.vala:276: %s", _tmp2_ = g_strconcat ("Could not load from ", string_to_string (filepath), NULL));
			_g_free0 (_tmp2_);
			_g_error_free0 (e);
		}
	}
...
static void __lambda17__unity_theme_file_path_found_icon_path (UnityThemeFilePath* _sender, const char* filepath, gpointer self) {
	_lambda17_ (_sender, filepath, self);
}
...
static void _lambda18_ (Block10Data* _data10_) {
	...
	__catch46_g_error:
	{
		...
		_inner_error_ = NULL;
		{
			g_warning ("scrollerchild-controller.vala:288: Could not load any icon for %s", _data10_->icon_name);
			_g_error_free0 (e);
		}
	}
	__finally46:
...
void unity_launcher_scroller_child_controller_load_icon_from_icon_name (UnityLauncherScrollerChildController* self, const char* icon_name) {
	...
	_data10_->icon_name = g_strdup (icon_name);
	if (unity_launcher_scroller_child_controller_try_load_from_file (self, _data10_->icon_name)) {
		block10_data_unref (_data10_);
		return;
	}
	if ((_tmp1_ = unity_launcher_scroller_child_controller_try_load_from_file (self, _tmp0_ = g_strconcat ("/usr/share/pixmaps/", _data10_->icon_name, NULL)), _g_free0 (_tmp0_), _tmp1_)) {
		block10_data_unref (_data10_);
		return;
	}
	...
	unity_theme_file_path_add_icon_theme (self->priv->theme_file_path, theme);
	g_signal_connect_object (self->priv->theme_file_path, "found-icon-path", (GCallback) __lambda17__unity_theme_file_path_found_icon_path, self, 0);
	g_signal_connect_data (self->priv->theme_file_path, "failed", (GCallback) __lambda18__unity_theme_file_path_failed, block10_data_ref (_data10_), (GClosureNotify) block10_data_unref, 0);
	unity_theme_file_path_get_icon_filepath (self->priv->theme_file_path, _data10_->icon_name, NULL, NULL);
...
static gboolean unity_launcher_scroller_child_controller_try_load_from_file (UnityLauncherScrollerChildController* self, const char* filepath) {
	...
	if (g_file_test (filepath, G_FILE_TEST_IS_REGULAR)) {
		...
		__catch47_g_error:
		{
			...
			_inner_error_ = NULL;
			{
				g_warning ("scrollerchild-controller.vala:306: Unable to load image from file '%s'" \
