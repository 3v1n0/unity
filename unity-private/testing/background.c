...
static void _unity_testing_background_update_image (UnityTestingBackground* self) {
	...
	__catch29_g_error:
	{
		...
		_inner_error_ = NULL;
		{
			g_warning ("background.vala:234: Background: Unable to load background file %s: %s", self->priv->filename, e->message);
			_g_error_free0 (e);
		}
	}
	__finally29:
...
static GObject * unity_testing_background_constructor (GType type, guint n_construct_properties, GObjectConstructParam * construct_properties) {
	...
	_inner_error_ = NULL;
	{
		...
		__catch35_g_error:
		{
			...
			_inner_error_ = NULL;
			{
				g_warning ("background.vala:63: Background: Unable to monitor background-settings:" \
" %s", e->message);
				_g_error_free0 (e);
			}
		}
		...
		__catch41_g_error:
		{
			...
			_inner_error_ = NULL;
			{
				g_warning ("background.vala:120: Background: Unable to monitor background: %s", e->message);
				_g_error_free0 (e);
			}
		}
		__finally41:
...
static void unity_testing_background_instance_init (UnityTestingBackground * self) {
	...
	self->priv->DEFAULT_RADIANCE_COL = g_strdup ("#AEAEA7A79F9F");
...
static void unity_testing_background_finalize (GObject* obj) {
	...
	_g_free0 (self->priv->DEFAULT_RADIANCE_COL);
