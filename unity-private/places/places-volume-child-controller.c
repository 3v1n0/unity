/* places-volume-child-controller.c generated by valac 0.9.5, the Vala compiler
...
static void unity_places_volume_child_controller_open_volume (UnityPlacesVolumeChildController* self) {
	...
	if (G_IS_MOUNT (mount)) {
		...
		__catch38_g_error:
		{
			...
			{
				char* _tmp1_;
				g_warning ("places-volume-child-controller.vala:86: %s", _tmp1_ = g_strconcat (error_msg, err->message, NULL));
				_g_free0 (_tmp1_);
				_g_error_free0 (err);
			}
		}
		...
		if (g_volume_can_mount (self->priv->_volume) == FALSE) {
			char* _tmp2_;
			g_warning ("places-volume-child-controller.vala:93: %s", _tmp2_ = g_strconcat (error_msg, "Cannot be mounted", NULL));
			_g_free0 (_tmp2_);
			_g_free0 (error_msg);
			_g_object_unref0 (mount);
			return;
		}
		{
			...
			if (G_IS_MOUNT (mount)) {
				...
				char* _tmp5_;
				g_warning ("places-volume-child-controller.vala:103: %s", _tmp5_ = g_strconcat (error_msg, "Unable to mount", NULL));
				_g_free0 (_tmp5_);
			}
		}
		goto __finally39;
		__catch39_g_error:
		{
			...
			{
				char* _tmp6_;
				g_warning ("places-volume-child-controller.vala:106: %s", _tmp6_ = g_strconcat (error_msg, e->message, NULL));
				_g_free0 (_tmp6_);
				_g_error_free0 (e);
			}
		}
...
static GObject * unity_places_volume_child_controller_constructor (GType type, guint n_construct_properties, GObjectConstructParam * construct_properties) {
	...
	self = UNITY_PLACES_VOLUME_CHILD_CONTROLLER (obj);
	{
		...
		g_debug ("places-volume-child-controller.vala:57: ICON NAME: %s", icon_name);
