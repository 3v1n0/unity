...
static void unity_places_place_entry_remote_dbus_proxy_set_global_search_async (UnityPlacesPlaceEntryRemote* self, const char* search_string, GHashTable* hints, GAsyncReadyCallback _callback_, gpointer _user_data_) {
	...
	const char* _tmp15_;
	DBusMessageIter _tmp16_, _tmp17_;
	GHashTableIter _tmp18_;
	gpointer _tmp19_, _tmp20_;
	...
	_tmp15_ = search_string;
	dbus_message_iter_append_basic (&_iter, DBUS_TYPE_STRING, &_tmp15_);
	dbus_message_iter_open_container (&_iter, DBUS_TYPE_ARRAY, "{ss}", &_tmp16_);
	g_hash_table_iter_init (&_tmp18_, hints);
	while (g_hash_table_iter_next (&_tmp18_, &_tmp19_, &_tmp20_)) {
		char* _key;
		char* _value;
		const char* _tmp21_;
		const char* _tmp22_;
		dbus_message_iter_open_container (&_tmp16_, DBUS_TYPE_DICT_ENTRY, NULL, &_tmp17_);
		_key = (char*) _tmp19_;
		_value = (char*) _tmp20_;
		_tmp21_ = _key;
		dbus_message_iter_append_basic (&_tmp17_, DBUS_TYPE_STRING, &_tmp21_);
		_tmp22_ = _value;
		dbus_message_iter_append_basic (&_tmp17_, DBUS_TYPE_STRING, &_tmp22_);
		dbus_message_iter_close_container (&_tmp16_, &_tmp17_);
	}
...
static void unity_places_place_entry_remote_dbus_proxy_set_search_async (UnityPlacesPlaceEntryRemote* self, const char* search, GHashTable* hints, GAsyncReadyCallback _callback_, gpointer _user_data_) {
	...
	dbus_message_iter_open_container (&_iter, DBUS_TYPE_ARRAY, "{ss}", &_tmp36_);
	g_hash_table_iter_init (&_tmp38_, hints);
...
static void unity_places_place_entry_remote_dbus_proxy_set_active_async (UnityPlacesPlaceEntryRemote* self, gboolean is_active, GAsyncReadyCallback _callback_, gpointer _user_data_) {
	...
	dbus_message_iter_init_append (_message, &_iter);
	_tmp52_ = is_active;
	dbus_message_iter_append_basic (&_iter, DBUS_TYPE_BOOLEAN, &_tmp52_);
	g_object_get (self, "connection", &_connection, NULL);
	dbus_connection_send_with_reply (dbus_g_connection_get_connection (_connection), _message, &_pending, -1);
	dbus_g_connection_unref (_connection);
	dbus_message_unref (_message);
...
static void unity_places_place_entry_remote_dbus_proxy_set_active_finish (UnityPlacesPlaceEntryRemote* self, GAsyncResult* _res_, GError** error) {
	...
	if (dbus_error_is_set (&_dbus_error)) {
		...
		if (strstr (_dbus_error.name, "org.freedesktop.DBus.Error") == _dbus_error.name) {
			const char* _tmp53_;
			_edomain = DBUS_GERROR;
			_tmp53_ = _dbus_error.name + 27;
			if (strcmp (_tmp53_, "Failed") == 0) {
				_ecode = DBUS_GERROR_FAILED;
			} else if (strcmp (_tmp53_, "NoMemory") == 0) {
				_ecode = DBUS_GERROR_NO_MEMORY;
			} else if (strcmp (_tmp53_, "ServiceUnknown") == 0) {
				_ecode = DBUS_GERROR_SERVICE_UNKNOWN;
			} else if (strcmp (_tmp53_, "NameHasNoOwner") == 0) {
				_ecode = DBUS_GERROR_NAME_HAS_NO_OWNER;
			} else if (strcmp (_tmp53_, "NoReply") == 0) {
				_ecode = DBUS_GERROR_NO_REPLY;
			} else if (strcmp (_tmp53_, "IOError") == 0) {
				_ecode = DBUS_GERROR_IO_ERROR;
			} else if (strcmp (_tmp53_, "BadAddress") == 0) {
				_ecode = DBUS_GERROR_BAD_ADDRESS;
			} else if (strcmp (_tmp53_, "NotSupported") == 0) {
				_ecode = DBUS_GERROR_NOT_SUPPORTED;
			} else if (strcmp (_tmp53_, "LimitsExceeded") == 0) {
				_ecode = DBUS_GERROR_LIMITS_EXCEEDED;
			} else if (strcmp (_tmp53_, "AccessDenied") == 0) {
				_ecode = DBUS_GERROR_ACCESS_DENIED;
			} else if (strcmp (_tmp53_, "AuthFailed") == 0) {
				_ecode = DBUS_GERROR_AUTH_FAILED;
			} else if (strcmp (_tmp53_, "NoServer") == 0) {
				_ecode = DBUS_GERROR_NO_SERVER;
			} else if (strcmp (_tmp53_, "Timeout") == 0) {
				_ecode = DBUS_GERROR_TIMEOUT;
			} else if (strcmp (_tmp53_, "NoNetwork") == 0) {
				_ecode = DBUS_GERROR_NO_NETWORK;
			} else if (strcmp (_tmp53_, "AddressInUse") == 0) {
				_ecode = DBUS_GERROR_ADDRESS_IN_USE;
			} else if (strcmp (_tmp53_, "Disconnected") == 0) {
				_ecode = DBUS_GERROR_DISCONNECTED;
			} else if (strcmp (_tmp53_, "InvalidArgs") == 0) {
				_ecode = DBUS_GERROR_INVALID_ARGS;
			} else if (strcmp (_tmp53_, "FileNotFound") == 0) {
				_ecode = DBUS_GERROR_FILE_NOT_FOUND;
			} else if (strcmp (_tmp53_, "FileExists") == 0) {
				_ecode = DBUS_GERROR_FILE_EXISTS;
			} else if (strcmp (_tmp53_, "UnknownMethod") == 0) {
				_ecode = DBUS_GERROR_UNKNOWN_METHOD;
			} else if (strcmp (_tmp53_, "TimedOut") == 0) {
				_ecode = DBUS_GERROR_TIMED_OUT;
			} else if (strcmp (_tmp53_, "MatchRuleNotFound") == 0) {
				_ecode = DBUS_GERROR_MATCH_RULE_NOT_FOUND;
			} else if (strcmp (_tmp53_, "MatchRuleInvalid") == 0) {
				_ecode = DBUS_GERROR_MATCH_RULE_INVALID;
			} else if (strcmp (_tmp53_, "Spawn.ExecFailed") == 0) {
				_ecode = DBUS_GERROR_SPAWN_EXEC_FAILED;
			} else if (strcmp (_tmp53_, "Spawn.ForkFailed") == 0) {
				_ecode = DBUS_GERROR_SPAWN_FORK_FAILED;
			} else if (strcmp (_tmp53_, "Spawn.ChildExited") == 0) {
				_ecode = DBUS_GERROR_SPAWN_CHILD_EXITED;
			} else if (strcmp (_tmp53_, "Spawn.ChildSignaled") == 0) {
				_ecode = DBUS_GERROR_SPAWN_CHILD_SIGNALED;
			} else if (strcmp (_tmp53_, "Spawn.Failed") == 0) {
				_ecode = DBUS_GERROR_SPAWN_FAILED;
			} else if (strcmp (_tmp53_, "UnixProcessIdUnknown") == 0) {
				_ecode = DBUS_GERROR_UNIX_PROCESS_ID_UNKNOWN;
			} else if (strcmp (_tmp53_, "InvalidSignature") == 0) {
				_ecode = DBUS_GERROR_INVALID_SIGNATURE;
			} else if (strcmp (_tmp53_, "InvalidFileContent") == 0) {
				_ecode = DBUS_GERROR_INVALID_FILE_CONTENT;
			} else if (strcmp (_tmp53_, "SELinuxSecurityContextUnknown") == 0) {
				_ecode = DBUS_GERROR_SELINUX_SECURITY_CONTEXT_UNKNOWN;
			} else if (strcmp (_tmp53_, "RemoteException") == 0) {
				_ecode = DBUS_GERROR_REMOTE_EXCEPTION;
			}
		}
		g_set_error (error, _edomain, _ecode, "%s", _dbus_error.message);
...
void unity_places_place_entry_dbus_place_entry_info_copy (const UnityPlacesPlaceEntryDbusPlaceEntryInfo* self, UnityPlacesPlaceEntryDbusPlaceEntryInfo* dest) {
	...
	dest->mimetype = (_tmp55_ = self->mimetype, (_tmp55_ == NULL) ? ((gpointer) _tmp55_) : _vala_array_dup2 (_tmp55_, (*self).mimetype_length1));
