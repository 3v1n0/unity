...
static gboolean _unity_places_view_on_stage_event_captured_clutter_actor_captured_event (ClutterActor* _sender, ClutterEvent* event, gpointer self) {
	gboolean result;
	result = unity_places_view_on_stage_event_captured (self, event);
	return result;
}
...
static gboolean unity_places_view_on_stage_event_captured (UnityPlacesView* self, ClutterEvent* event) {
	...
	if ((*event).type == CLUTTER_KEY_PRESS) {
		...
		ev = (*event).key;
		if (ev.keyval == 0xff1b) {
			if (self->search_bar->sections->active_section_n != 0) {
				unity_places_place_search_sections_bar_set_active_section (self->search_bar->sections, (guint) 0);
			} else {
				unity_shell_hide_unity (self->priv->_shell);
			}
			result = TRUE;
			return result;
		}
...
static GObject * unity_places_view_constructor (GType type, guint n_construct_properties, GObjectConstructParam * construct_properties) {
	...
	self = UNITY_PLACES_VIEW (obj);
	{
		...
		g_signal_connect_object ((ClutterActor*) (_tmp0_ = unity_shell_get_stage (self->priv->_shell)), "captured-event", (GCallback) _unity_places_view_on_stage_event_captured_clutter_actor_captured_event, self, 0);
