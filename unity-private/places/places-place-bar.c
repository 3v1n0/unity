...
static void _lambda74_ (UnityPlacesPlace* p, UnityPlacesPlaceBar* self);
static void __lambda74__unity_places_place_model_place_added (UnityPlacesPlaceModel* _sender, UnityPlacesPlace* place, gpointer self);
...
static GObject * unity_places_place_bar_constructor (GType type, guint n_construct_properties, GObjectConstructParam * construct_properties) {
	...
	self = UNITY_PLACES_PLACE_BAR (obj);
	{
		...
		g_signal_connect_object (self->priv->_model, "place-added", (GCallback) __lambda74__unity_places_place_model_place_added, self, 0);
	}
	return obj;
}
