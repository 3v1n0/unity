/* places-place-search-entry.c generated by valac 0.9.7, the Vala compiler
...
static gboolean _lambda35_ (UnityPlacesPlaceSearchEntry* self);
...
static gboolean _lambda36_ (UnityPlacesPlaceSearchEntry* self);
...
static void unity_places_place_search_entry_on_text_changed (UnityPlacesPlaceSearchEntry* self) {
	...
	self->priv->live_search_timeout = g_timeout_add_full (G_PRIORITY_DEFAULT, (guint) UNITY_PLACES_PLACE_SEARCH_ENTRY_LIVE_SEARCH_TIMEOUT, __lambda35__gsource_func, g_object_ref (self), g_object_unref);
}
...
static GObject * unity_places_place_search_entry_constructor (GType type, guint n_construct_properties, GObjectConstructParam * construct_properties) {
	...
	self = UNITY_PLACES_PLACE_SEARCH_ENTRY (obj);
	{
		...
		g_signal_connect_object ((ClutterActor*) self->right_icon, "button-release-event", (GCallback) __lambda36__clutter_actor_button_release_event, self, 0);
	}
	return obj;
}
