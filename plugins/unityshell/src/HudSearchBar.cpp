  ...
  , live_search_timeout_(0)
{
  ...
  spinner_ = new unity::dash::SearchBarSpinner();
  ...
  layout_->AddView(spinner_, 0, nux::MINOR_POSITION_TOP, nux::MINOR_SIZE_FULL);

  hint_ = new nux::StaticCairoText(" ");
  hint_->SetTextColor(nux::Color(1.0f, 1.0f, 1.0f, 0.5f));
  //hint_->SetMaximumWidth(570);

  pango_entry_ = new unity::dash::IMTextEntry();
...
void SearchBar::OnSearchHintChanged()
{
  ...
  gchar* markup  = g_strdup_printf("%s", tmp);
...
void SearchBar::OnSearchChanged(nux::TextEntry* text_entry)
{
  ...
  //spinner_->SetState(is_empty ? unity::dash::STATE_READY : unity::dash::STATE_SEARCHING);
                                     ...
                                     unsigned long key_fags)
{
  ...
  spinner_->SetState(unity::dash::STATE_READY);
...
{
  spinner_->SetState(unity::dash::STATE_CLEAR);
}
...
bool SearchBar::set_search_string(std::string const& string)
{
  ...
  spinner_->SetState(string == "" ? unity::dash::STATE_READY : unity::dash::STATE_CLEAR);
  return true;
}
