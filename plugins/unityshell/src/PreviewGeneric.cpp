...
namespace unity {
    ...
    void PreviewGeneric::BuildLayout()
    {
      ...
      if (preview_->tertiary_action_name.empty() == false)
      {
        ...
        tertiary_button->click.connect ([&] (nux::View *view) { UriActivated.emit (preview_->tertiary_action_uri); });
      ...
      if (preview_->secondary_action_name.empty() == false)
      {
        ...
        secondary_button->click.connect ([&] (nux::View *view) { UriActivated.emit (preview_->secondary_action_uri); });
        button_container->AddLayout (new nux::SpaceLayout(6,6,6,6), 0);
