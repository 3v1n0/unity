...
unity_places_simple_tile_accessible_new(nux::Object* object)
{
  ...
  g_return_val_if_fail(dynamic_cast<PlacesSimpleTile*>(object), NULL);
  ...
  if (name == NULL)
    {
      ...
      tile = dynamic_cast<PlacesSimpleTile*>(nux_object_accessible_get_object(NUX_OBJECT_ACCESSIBLE(obj)));
      if (tile != NULL)
        {
          name = tile->GetLabel();
