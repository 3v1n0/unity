...
namespace
{
nux::logging::Logger logger("unity.dash.radiobuttonfilter");
}

RadioButtonFilter::RadioButtonFilter(DeeModel* model, DeeModelIter* iter)
  : Filter(model, iter)
{
  buttons.SetGetterFunction(sigc::mem_fun(this, &RadioButtonFilter::get_buttons));
  Refresh();
}

void RadioButtonFilter::Clear()
{
...
void RadioButtonFilter::Toggle(std::string id)
{
  for(auto button: buttons_)
  {
    if (button->id == id)
...
void RadioButtonFilter::Update(Filter::Hints& hints)
{
...
RadioButtonFilter::RadioButtons const& RadioButtonFilter::get_buttons() const
{
  return buttons_;
}

void RadioButtonFilter::UpdateState(bool raw_filtering)
{
  if (!IsValid())
    return;
