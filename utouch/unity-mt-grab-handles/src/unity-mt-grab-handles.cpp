...
UnityMTGrabHandlesWindow::getOutputExtents (CompWindowExtents &output)
{
	...
	output.left   = MAX (output.left, window->input ().left + mHandles->at (0).width () / 2 + 20); /* fix me sam, you're my only hope */
	output.right  = MAX (output.right, window->input ().right + mHandles->at (0).width () / 2 + 20); /* the + 20 I mean */
	output.top    = MAX (output.top, window->input ().top  + mHandles->at (0).height () / 2 + 20); /* whenever you get a chance */
	output.bottom = MAX (output.bottom, window->input ().bottom + mHandles->at (0).height () / 2 + 20); /* nothing left to say really... */
    }
    else
