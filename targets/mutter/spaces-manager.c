/* spaces-manager.c generated by valac 0.9.5, the Vala compiler
...
static gboolean __lambda12__clutter_actor_button_release_event (ClutterActor* _sender, ClutterEvent* event, gpointer self);
static gboolean _lambda13_ (Block2Data* _data2_);
static gboolean __lambda13__clutter_actor_enter_event (ClutterActor* _sender, ClutterEvent* event, gpointer self);
static gboolean _lambda14_ (Block2Data* _data2_);
static gboolean __lambda14__clutter_actor_leave_event (ClutterActor* _sender, ClutterEvent* event, gpointer self);
	...
	clutter_actor_set_reactive (self->priv->background, TRUE);
	{
		...
		for (workspace_it = workspace_collection; workspace_it != NULL; workspace_it = workspace_it->next) {
			...
			workspace = (MetaWorkspace*) workspace_it->data;
			{
				...
				g_signal_connect_data (_data2_->clone, "button-release-event", (GCallback) __lambda12__clutter_actor_button_release_event, block2_data_ref (_data2_), (GClosureNotify) block2_data_unref, 0);
				g_signal_connect_data (_data2_->clone, "enter-event", (GCallback) __lambda13__clutter_actor_enter_event, block2_data_ref (_data2_), (GClosureNotify) block2_data_unref, 0);
				g_signal_connect_data (_data2_->clone, "leave-event", (GCallback) __lambda14__clutter_actor_leave_event, block2_data_ref (_data2_), (GClosureNotify) block2_data_unref, 0);
				...
				}
				{
					...
					x = 0;
					{
						...
						while (TRUE) {
							...
							unity_workspace_clone_ungrid ((_tmp3_ = _data4_->clone, UNITY_IS_WORKSPACE_CLONE (_tmp3_) ? ((UnityWorkspaceClone*) _tmp3_) : NULL));
							g_signal_connect_data (anim, "completed", (GCallback) __lambda9__clutter_animation_completed, block4_data_ref (_data4_), (GClosureNotify) block4_data_unref, 0);
