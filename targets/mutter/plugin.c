/* plugin.c generated by valac 0.9.5, the Vala compiler
...
static gboolean _lambda15_ (UnityPlugin* self);
...
static void _lambda19_ (ClutterAnimation* an, UnityPlugin* self);
...
static gboolean unity_plugin_real_construct (UnityPlugin* self) {
	...
	if (boot_logging_filename != NULL) {
		g_timeout_add_seconds_full (G_PRIORITY_DEFAULT, (guint) 5, __lambda15__gsource_func, g_object_ref (self), g_object_unref);
	}
	unity_shell_ensure_input_region ((UnityShell*) self);
	result = FALSE;
...
static void unity_plugin_check_fullscreen_obstruction (UnityPlugin* self) {
	...
	if (CLUTTER_IS_ANIMATION (panim)) {
		g_signal_connect_object (panim, "completed", (GCallback) __lambda17__clutter_animation_completed, self, 0);
	}
...
static void unity_plugin_real_hide_unity (UnityShell* base) {
	...
	g_signal_connect_object (anim, "completed", (GCallback) __lambda19__clutter_animation_completed, self, 0);
	...
	if (win != NULL) {
		switch (action) {
			...
			default:
			{
				char* _tmp1_;
				GEnumValue* _tmp0_;
				g_warning ("plugin.vala:767: %s", _tmp1_ = g_strconcat ("Window action type ", (_tmp0_ = g_enum_get_value (g_type_class_ref (UNITY_TYPE_WINDOW_ACTION), (int) action), (_tmp0_ != NULL) ? _tmp0_->value_name : NULL), " not supported", NULL));
				_g_free0 (_tmp1_);
				break;
			}
		}
...
static gboolean _lambda20_ (Block5Data* _data5_) {
	...
	gboolean result = FALSE;
	self = _data5_->self;
	if (G_IS_OBJECT (_data5_->win)) {
		gboolean _tmp0_ = FALSE;
		if (utils_window_is_decorated (meta_window_get_xwindow (_data5_->win)) == FALSE) {
			_tmp0_ = meta_window_is_maximized (_data5_->win) == FALSE;
		} else {
			_tmp0_ = FALSE;
		}
		if (_tmp0_) {
			g_object_set_data_full ((GObject*) _data5_->window, UNITY_PLUGIN_UNDECORATED_HINT, g_strdup_printf ("%s", "true"), g_free);
		} else {
			if (meta_window_is_maximized (_data5_->win)) {
				utils_window_set_decorations (meta_window_get_xwindow (_data5_->win), (guint) 0);
			}
		}
	}
	result = FALSE;
...
void unity_plugin_map (UnityPlugin* self, MutterWindow* window) {
	...
	_data5_->window = _g_object_ref0 (window);
	_data5_->win = mutter_window_get_meta_window (_data5_->window);
	if (mutter_window_get_window_type (_data5_->window) == META_COMP_WINDOW_NORMAL) {
		g_idle_add_full (G_PRIORITY_DEFAULT_IDLE, __lambda20__gsource_func, block5_data_ref (_data5_), block5_data_unref);
	}
	unity_maximus_process_window (self->priv->maximus, _data5_->window);
	g_signal_emit_by_name (self, "window-mapped", self, _data5_->window);
	if (self->priv->display == NULL) {
		self->priv->display = meta_window_get_display (mutter_window_get_meta_window (_data5_->window));
...
static GObject * unity_plugin_constructor (GType type, guint n_construct_properties, GObjectConstructParam * construct_properties) {
	...
	_inner_error_ = NULL;
	{
		...
		__catch0_g_error:
		{
			...
			_inner_error_ = NULL;
			{
				g_warning ("plugin.vala:200: %s", e->message);
				_g_error_free0 (e);
			}
		}
		__finally0:
