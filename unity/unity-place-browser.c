/* unity-place-browser.c generated by valac 0.9.7, the Vala compiler
...
static UnityPlace_BrowsingState* unity_place_browser_service_dbus_proxy_go_back (UnityPlaceBrowserService* self, int* result_length1, GError** error) {
	...
	int _result_length1;
	UnityPlace_BrowsingState* _tmp140_;
	int _tmp140__length;
	int _tmp140__size;
	int _tmp140__length1;
	DBusMessageIter _tmp141_;
	if (((UnityPlaceBrowserServiceDBusProxy*) self)->disposed) {
	...
	_result_length1 = 0;
	_tmp140_ = g_new (UnityPlace_BrowsingState, 5);
	_tmp140__length = 0;
	_tmp140__size = 4;
	_tmp140__length1 = 0;
	dbus_message_iter_recurse (&_iter, &_tmp141_);
	for (; dbus_message_iter_get_arg_type (&_tmp141_); _tmp140__length1++) {
		UnityPlace_BrowsingState _tmp142_;
		DBusMessageIter _tmp143_;
		dbus_bool_t _tmp144_;
		const char* _tmp145_;
		if (_tmp140__size == _tmp140__length) {
			_tmp140__size = 2 * _tmp140__size;
			_tmp140_ = g_renew (UnityPlace_BrowsingState, _tmp140_, _tmp140__size + 1);
		}
		dbus_message_iter_recurse (&_tmp141_, &_tmp143_);
		dbus_message_iter_get_basic (&_tmp143_, &_tmp144_);
		dbus_message_iter_next (&_tmp143_);
		_tmp142_.sensitive = _tmp144_;
		dbus_message_iter_get_basic (&_tmp143_, &_tmp145_);
		dbus_message_iter_next (&_tmp143_);
		_tmp142_.comment = g_strdup (_tmp145_);
		dbus_message_iter_next (&_tmp141_);
		_tmp140_[_tmp140__length++] = _tmp142_;
	}
	_result_length1 = _tmp140__length1;
...
static UnityPlace_BrowsingState* unity_place_browser_service_dbus_proxy_go_forward (UnityPlaceBrowserService* self, int* result_length1, GError** error) {
	...
	UnityPlace_BrowsingState* _result;
	int _result_length1;
	UnityPlace_BrowsingState* _tmp147_;
	int _tmp147__length;
	int _tmp147__size;
	int _tmp147__length1;
	...
	if (dbus_error_is_set (&_dbus_error)) {
		...
		if (strstr (_dbus_error.name, "org.freedesktop.DBus.Error") == _dbus_error.name) {
			const char* _tmp153_;
			_edomain = DBUS_GERROR;
			_tmp153_ = _dbus_error.name + 27;
			if (strcmp (_tmp153_, "Failed") == 0) {
				_ecode = DBUS_GERROR_FAILED;
			} else if (strcmp (_tmp153_, "NoMemory") == 0) {
				_ecode = DBUS_GERROR_NO_MEMORY;
			} else if (strcmp (_tmp153_, "ServiceUnknown") == 0) {
				_ecode = DBUS_GERROR_SERVICE_UNKNOWN;
			} else if (strcmp (_tmp153_, "NameHasNoOwner") == 0) {
				_ecode = DBUS_GERROR_NAME_HAS_NO_OWNER;
			} else if (strcmp (_tmp153_, "NoReply") == 0) {
				_ecode = DBUS_GERROR_NO_REPLY;
			} else if (strcmp (_tmp153_, "IOError") == 0) {
				_ecode = DBUS_GERROR_IO_ERROR;
			} else if (strcmp (_tmp153_, "BadAddress") == 0) {
				_ecode = DBUS_GERROR_BAD_ADDRESS;
			} else if (strcmp (_tmp153_, "NotSupported") == 0) {
				_ecode = DBUS_GERROR_NOT_SUPPORTED;
			} else if (strcmp (_tmp153_, "LimitsExceeded") == 0) {
				_ecode = DBUS_GERROR_LIMITS_EXCEEDED;
			} else if (strcmp (_tmp153_, "AccessDenied") == 0) {
				_ecode = DBUS_GERROR_ACCESS_DENIED;
			} else if (strcmp (_tmp153_, "AuthFailed") == 0) {
				_ecode = DBUS_GERROR_AUTH_FAILED;
			} else if (strcmp (_tmp153_, "NoServer") == 0) {
				_ecode = DBUS_GERROR_NO_SERVER;
			} else if (strcmp (_tmp153_, "Timeout") == 0) {
				_ecode = DBUS_GERROR_TIMEOUT;
			} else if (strcmp (_tmp153_, "NoNetwork") == 0) {
				_ecode = DBUS_GERROR_NO_NETWORK;
			} else if (strcmp (_tmp153_, "AddressInUse") == 0) {
				_ecode = DBUS_GERROR_ADDRESS_IN_USE;
			} else if (strcmp (_tmp153_, "Disconnected") == 0) {
				_ecode = DBUS_GERROR_DISCONNECTED;
			} else if (strcmp (_tmp153_, "InvalidArgs") == 0) {
				_ecode = DBUS_GERROR_INVALID_ARGS;
			} else if (strcmp (_tmp153_, "FileNotFound") == 0) {
				_ecode = DBUS_GERROR_FILE_NOT_FOUND;
			} else if (strcmp (_tmp153_, "FileExists") == 0) {
				_ecode = DBUS_GERROR_FILE_EXISTS;
			} else if (strcmp (_tmp153_, "UnknownMethod") == 0) {
				_ecode = DBUS_GERROR_UNKNOWN_METHOD;
			} else if (strcmp (_tmp153_, "TimedOut") == 0) {
				_ecode = DBUS_GERROR_TIMED_OUT;
			} else if (strcmp (_tmp153_, "MatchRuleNotFound") == 0) {
				_ecode = DBUS_GERROR_MATCH_RULE_NOT_FOUND;
			} else if (strcmp (_tmp153_, "MatchRuleInvalid") == 0) {
				_ecode = DBUS_GERROR_MATCH_RULE_INVALID;
			} else if (strcmp (_tmp153_, "Spawn.ExecFailed") == 0) {
				_ecode = DBUS_GERROR_SPAWN_EXEC_FAILED;
			} else if (strcmp (_tmp153_, "Spawn.ForkFailed") == 0) {
				_ecode = DBUS_GERROR_SPAWN_FORK_FAILED;
			} else if (strcmp (_tmp153_, "Spawn.ChildExited") == 0) {
				_ecode = DBUS_GERROR_SPAWN_CHILD_EXITED;
			} else if (strcmp (_tmp153_, "Spawn.ChildSignaled") == 0) {
				_ecode = DBUS_GERROR_SPAWN_CHILD_SIGNALED;
			} else if (strcmp (_tmp153_, "Spawn.Failed") == 0) {
				_ecode = DBUS_GERROR_SPAWN_FAILED;
			} else if (strcmp (_tmp153_, "UnixProcessIdUnknown") == 0) {
				_ecode = DBUS_GERROR_UNIX_PROCESS_ID_UNKNOWN;
			} else if (strcmp (_tmp153_, "InvalidSignature") == 0) {
				_ecode = DBUS_GERROR_INVALID_SIGNATURE;
			} else if (strcmp (_tmp153_, "InvalidFileContent") == 0) {
				_ecode = DBUS_GERROR_INVALID_FILE_CONTENT;
			} else if (strcmp (_tmp153_, "SELinuxSecurityContextUnknown") == 0) {
				_ecode = DBUS_GERROR_SELINUX_SECURITY_CONTEXT_UNKNOWN;
			} else if (strcmp (_tmp153_, "RemoteException") == 0) {
				_ecode = DBUS_GERROR_REMOTE_EXCEPTION;
			}
		}
		g_set_error (error, _edomain, _ecode, "%s", _dbus_error.message);
	...
	dbus_message_iter_init (_reply, &_iter);
	_result_length1 = 0;
	_tmp147_ = g_new (UnityPlace_BrowsingState, 5);
	_tmp147__length = 0;
	_tmp147__size = 4;
	_tmp147__length1 = 0;
	dbus_message_iter_recurse (&_iter, &_tmp148_);
	for (; dbus_message_iter_get_arg_type (&_tmp148_); _tmp147__length1++) {
		UnityPlace_BrowsingState _tmp149_;
		DBusMessageIter _tmp150_;
		dbus_bool_t _tmp151_;
		const char* _tmp152_;
		if (_tmp147__size == _tmp147__length) {
			_tmp147__size = 2 * _tmp147__size;
			_tmp147_ = g_renew (UnityPlace_BrowsingState, _tmp147_, _tmp147__size + 1);
		}
		dbus_message_iter_recurse (&_tmp148_, &_tmp150_);
		dbus_message_iter_get_basic (&_tmp150_, &_tmp151_);
		dbus_message_iter_next (&_tmp150_);
		_tmp149_.sensitive = _tmp151_;
		dbus_message_iter_get_basic (&_tmp150_, &_tmp152_);
		dbus_message_iter_next (&_tmp150_);
		_tmp149_.comment = g_strdup (_tmp152_);
		dbus_message_iter_next (&_tmp148_);
		_tmp147_[_tmp147__length++] = _tmp149_;
	}
	_result_length1 = _tmp147__length1;
	dbus_message_iter_next (&_iter);
	_result = _tmp147_;
	*result_length1 = _result_length1;
	dbus_message_unref (_reply);
...
static UnityPlace_BrowsingState* unity_place_browser_service_dbus_proxy_get_state (UnityPlaceBrowserService* self, int* result_length1, GError** error) {
	...
	UnityPlace_BrowsingState* _result;
	int _result_length1;
	UnityPlace_BrowsingState* _tmp154_;
	int _tmp154__length;
	int _tmp154__size;
	...
	dbus_message_iter_init (_reply, &_iter);
	_result_length1 = 0;
	_tmp154_ = g_new (UnityPlace_BrowsingState, 5);
	_tmp154__length = 0;
	_tmp154__size = 4;
	_tmp154__length1 = 0;
	dbus_message_iter_recurse (&_iter, &_tmp155_);
	for (; dbus_message_iter_get_arg_type (&_tmp155_); _tmp154__length1++) {
		UnityPlace_BrowsingState _tmp156_;
		DBusMessageIter _tmp157_;
		dbus_bool_t _tmp158_;
		const char* _tmp159_;
		if (_tmp154__size == _tmp154__length) {
			_tmp154__size = 2 * _tmp154__size;
			_tmp154_ = g_renew (UnityPlace_BrowsingState, _tmp154_, _tmp154__size + 1);
		}
		dbus_message_iter_recurse (&_tmp155_, &_tmp157_);
		dbus_message_iter_get_basic (&_tmp157_, &_tmp158_);
		dbus_message_iter_next (&_tmp157_);
		_tmp156_.sensitive = _tmp158_;
		dbus_message_iter_get_basic (&_tmp157_, &_tmp159_);
		dbus_message_iter_next (&_tmp157_);
		_tmp156_.comment = g_strdup (_tmp159_);
		dbus_message_iter_next (&_tmp155_);
		_tmp154_[_tmp154__length++] = _tmp156_;
	}
...
static UnityPlace_BrowsingState* _vala_array_dup8 (UnityPlace_BrowsingState* self, int length) {
	...
	for (i = 0; i < length; i++) {
		result[i] = (unity_place__browsingstate_copy (&self[i], &_tmp0_), _tmp0_);
	}
	return result;
}
