...
void UnityWindowView::OnDPIChanged()
{
  ...
  if (internal_layout_)
  {
    int offset = RawPixel(style()->GetInternalOffset()).CP(scale);
...
void UnityWindowView::OnClosableChanged(bool closable)
{
  ...
  int padding = RawPixel(style()->GetCloseButtonPadding()).CP(scale);
...
{
  int border = RawPixel(style()->GetBorderSize()).CP(scale);
